...
class ImplicitQLearning:
    ...
    def train(self, rb, invest=False) -> Dict[str, float]:
        self.total_it += 1
...
def train(config: TrainConfig):
    ...
    run_name = f"{config.alg}_{config.env}_invest_w{config.invest_window_size}i{config.invest_interval}"
    ...
    for t in range(int(config.max_timesteps)):
        ...
        log_dict = trainer.train(replay_buffer, invest=invest)
        if (t + 1) % 5000 == 0:
