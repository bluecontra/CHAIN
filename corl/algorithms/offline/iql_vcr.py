...
class GaussianPolicy(nn.Module):
    ...
    def get_mean(self, obs: torch.Tensor):
        mean = self.net(obs)
        return mean
...
class ImplicitQLearning:
    def __init__(
        ...
        # - churn reduction hyperparameters
        ...
        self.init_p = copy.deepcopy(self.actor)
        self.init_vf = copy.deepcopy(self.vf)
        self.init_qf = copy.deepcopy(self.qf)
        self.his_policy_list = [self.init_p]
        self.his_vf_list = [self.init_vf]
        self.his_qf_list = [self.init_qf]
    ...
    def _update_q(
        ...
        if vcr:
            if len(self.his_qf_list) <= self.reg_his_idx:
                q_reg_loss = 0
                log_dict["q_reg_loss"] = 0
            else:
                (
                    reg_observations,
                    reg_actions,
                    reg_rewards,
                    reg_next_observations,
                    reg_dones,
                ) = reg_data
                cur_qs = self.qf.both(reg_observations, reg_actions)
                reg_qf = self.his_qf_list[-self.reg_his_idx]
                with torch.no_grad():
                    reg_qs = reg_qf.both(reg_observations, reg_actions)
                q_reg_loss = sum(F.mse_loss(cur_q, reg_q) for cur_q, reg_q in zip(cur_qs, reg_qs)) / len(cur_qs)
                log_dict["q_reg_loss"] = q_reg_loss.item()
            total_loss = q_loss + self.v_reg_coef * q_reg_loss
        else:
            total_loss = q_loss
    ...
    def train(self, rb, invest=False, vcr=False) -> Dict[str, float]:
        self.total_it += 1
        ...
        if vcr:
            reg_batch = rb.sample(self.batch_size)
            reg_batch = [b.to(self.device) for b in reg_batch]
        else:
            reg_batch = None
        ...
        # Update Q function
        self._update_q(next_v, observations, actions, rewards, dones, log_dict, reg_data=reg_batch, vcr=vcr)
        # Update actor
        ...
        if invest:
            ...
            self.invest_policy_and_value_churn(ref_batch, log_dict)

        # NOTE - record historical policies and vfs
        self.his_policy_list.append(copy.deepcopy(self.actor))
        self.his_vf_list.append(copy.deepcopy(self.vf))
        self.his_qf_list.append(copy.deepcopy(self.qf))
        self.his_policy_list = self.his_policy_list[-self.invest_window_size * self.invest_interval:]
        self.his_vf_list = self.his_vf_list[-self.invest_window_size * self.invest_interval:]
    ...
    def invest_policy_and_value_churn(self, ref_data, log_dict):
        ...
        with torch.no_grad():
            ...
            for p, vf in zip(ref_policy_list, ref_vf_list):
                ...
                his_ref_v = vf(ref_observations)
                prior_ref_policy_list.append(his_ref_p)
                prior_ref_v_list.append(his_ref_v)

            # Calculate outputs of frozen init policy
            init_ref_p = self.init_p.get_mean(ref_observations)
            init_ref_v = self.init_vf(ref_observations)
            prior_ref_policy_list.append(init_ref_p)
            prior_ref_v_list.append(init_ref_v)

        # Calculate prior-post update diff
        cnt = 0
        for ref_p, ref_v in zip(prior_ref_policy_list, prior_ref_v_list):
            ...
            invest_v_diff = (cur_ref_v - ref_v).mean()
            invest_v_diff_abs = (cur_ref_v - ref_v).abs().mean()

            if cnt == 0:
                write_suffix = ''
            elif cnt == len(prior_ref_v_list) - 1:
                write_suffix = '_init'
            else:
            ...
            log_dict["invest/v_diff_sig" + write_suffix] = invest_v_diff.item()
            log_dict["invest/v_diff_abs" + write_suffix] = invest_v_diff_abs.item()
...
def train(config: TrainConfig):
    ...
    kwargs = {
        ...
        # NOTE - regularization specific arguments
        "reg_his_idx": config.reg_his_idx,
        "v_reg_coef": config.v_reg_coef,
    }
    ...
    run_name = f"{config.alg}_{config.env}"
    if config.alg in ['iql_vcr',]:
        run_name += f"_v{config.v_reg_coef}"
    run_name += f"_invest_w{config.invest_window_size}i{config.invest_interval}"
    ...
    for t in range(int(config.max_timesteps)):
        ...
        vcr = True if config.alg in ['iql_vcr',] else False
        log_dict = trainer.train(replay_buffer, invest=invest, vcr=vcr)
        if (t + 1) % 5000 == 0:
