...
class ImplicitQLearning:
    ...
    def _update_policy(
        ...
        if pcr:
            if len(self.his_policy_list) <= self.reg_his_idx:
                ...
                cur_action_means = self.actor.get_mean(reg_observations)
                reg_policy = self.his_policy_list[-self.reg_his_idx]
                with torch.no_grad():
                    reg_action_means = reg_policy.get_mean(reg_observations)
                reg_loss = ((cur_action_means - reg_action_means) ** 2).mean()
                log_dict["reg_loss"] = reg_loss.item()
            total_loss = policy_loss + self.reg_coef * reg_loss
        else:
    ...
    def train(self, rb, invest=False, pcr=False) -> Dict[str, float]:
        self.total_it += 1
        ...
        if pcr:
            ...
            reg_batch = None
        self._update_policy(adv, observations, actions, log_dict, reg_data=reg_batch, pcr=pcr)
...
def train(config: TrainConfig):
    ...
    if config.alg in ['iql_pcr',]:
        run_name += f"_p{config.reg_coef}"
    ...
    for t in range(int(config.max_timesteps)):
        ...
        pcr = True if config.alg in ['iql_pcr',] else False
        log_dict = trainer.train(replay_buffer, invest=invest, pcr=pcr)
        if (t + 1) % 5000 == 0:
