...
class AdvantageWeightedActorCritic:
    def __init__(
        ...
        self.init_c1 = copy.deepcopy(self._critic_1)
        self.init_c2 = copy.deepcopy(self._critic_2)
        self.his_policy_list = [self.init_policy]
        self.his_critic1_list = [self.init_c1]
        self.his_critic2_list = [self.init_c2]
    ...
    def _update_critic(self, states, actions, rewards, dones, next_states, log_dict: Dict, reg_data=None, vcr=False):
        critic_loss = self._critic_loss(states, actions, rewards, dones, next_states)
        log_dict["critic_loss"] = critic_loss.item()

        # NOTE - compute churn reduction loss
        if vcr:
            if len(self.his_critic1_list) <= self.reg_his_idx:
                reg_loss = 0
                log_dict["v_reg_loss"] = 0
            else:
                reg_observations, reg_actions, _, _, _ = reg_data
                cur_q1 = self._critic_1(reg_observations, reg_actions)
                cur_q2 = self._critic_2(reg_observations, reg_actions)
                reg_c1 = self.his_critic1_list[-self.reg_his_idx]
                reg_c2 = self.his_critic2_list[-self.reg_his_idx]
                with torch.no_grad():
                    reg_q1 = reg_c1(reg_observations, reg_actions)
                    reg_q2 = reg_c2(reg_observations, reg_actions)
                    reg_q = torch.min(reg_q1, reg_q2)
                reg_loss = nn.functional.mse_loss(cur_q1, reg_q) + nn.functional.mse_loss(cur_q2, reg_q)
                log_dict["v_reg_loss"] = reg_loss.item()
            total_loss = critic_loss + self.v_reg_coef * reg_loss
        else:
            total_loss = critic_loss

        log_dict["total_critic_loss"] = total_loss.item()
    ...
    def update(self, rb, invest=False, vcr=False) -> Dict[str, float]:
        ...
        critic_loss = self._update_critic(states, actions, rewards, dones, next_states,
                                          log_dict=result, reg_data=reg_batch, vcr=vcr)
        ...
        self.his_critic1_list.append(copy.deepcopy(self._critic_1))
        self.his_critic2_list.append(copy.deepcopy(self._critic_2))
        self.his_policy_list = self.his_policy_list[-self.invest_window_size * self.invest_interval:]
        self.his_critic1_list = self.his_critic1_list[-self.invest_window_size * self.invest_interval:]
        self.his_critic2_list = self.his_critic2_list[-self.invest_window_size * self.invest_interval:]
    ...
    def invest_policy_and_value_churn(self, ref_data, log_dict):
        ...
        frozen_vf = self.his_critic1_list[-1]
        ref_policy_list, ref_vf_list = [], []
        for i in range(self.invest_window_size):
            if (i + 1) * self.invest_interval > len(self.his_critic1_list):
                break
            ref_policy_list.append(self.his_policy_list[-(i + 1) * self.invest_interval])
            ref_vf_list.append(self.his_critic1_list[-(i + 1) * self.invest_interval])
        ...
        with torch.no_grad():
            ...
            init_ref_q = self.init_c1(ref_observations, ref_actions)
...
def train(config: TrainConfig):
    ...
    run_name = f"{config.alg}_{config.env_name}"
    if config.alg in ['awac_vcr',]:
    ...
    for t in trange(config.num_train_ops, ncols=80):
        ...
        vcr = True if config.alg == 'awac_vcr' else False
        update_result = awac.update(replay_buffer, invest=invest, vcr=vcr)
        if (t + 1) % 5000 == 0:
