...
class AdvantageWeightedActorCritic:
    ...
    def _update_actor(self, states, actions, log_dict: Dict, reg_data=None, pcr=False, reg_scale=False):
        policy_loss = self._actor_loss(states, actions)
        log_dict["actor_loss"] = policy_loss.item()
        reg_loss_scale = 1 if not reg_scale else policy_loss.abs().detach()
        ...
        if pcr:
            if len(self.his_policy_list) <= self.reg_his_idx:
                ...
                reg_observations, reg_actions, reg_rewards, reg_next_observations, reg_dones = reg_data
                cur_action_means = self._actor.get_mean(reg_observations)
            ...
            total_loss = policy_loss + self.reg_coef * reg_loss * reg_loss_scale
        else:
    ...
    def update(self, rb, invest=False, pcr=False, reg_scale=False) -> Dict[str, float]:
        ...
        if pcr:
            ...
            reg_batch = None
        self._update_actor(states, actions, log_dict=result, reg_data=reg_batch, pcr=pcr, reg_scale=reg_scale)
...
def train(config: TrainConfig):
    ...
    for t in trange(config.num_train_ops, ncols=80):
        ...
        pcr = True if config.alg == 'awac_pcr' else False
        reg_scale = False if config.reg_scale == 0 else True
        update_result = awac.update(replay_buffer, invest=invest, pcr=pcr, reg_scale=reg_scale)
        if (t + 1) % 5000 == 0:
