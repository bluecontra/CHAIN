...
import copy
from utils.basic_utils import make_env, Agent, eval_policy


@dataclass
class Args:
    scale_up_ratio: int = 1
    target_rel_loss_scale: float = 0.02

    # Algorithm specific arguments
    alg: str = 'ppo_chain_auto'
    """the name of designated algorithm"""
    env_id: str = "dm_control/ball_in_cup-catch-v0"
...
if __name__ == "__main__":
    ...
    eval_envs = gym.vector.SyncVectorEnv([make_env(args.env_id, 0, args.capture_video, run_name, args.gamma)])

    agent = Agent(envs, scale_up_ratio=args.scale_up_ratio).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        writer.add_scalar("losses/reg_loss", reg_loss.item(), global_step)
        writer.add_scalar("charts/auto_coef", cur_reg_coef, global_step)
        ...
        # NOTE - adjust reg coef according to the target relative loss scale
