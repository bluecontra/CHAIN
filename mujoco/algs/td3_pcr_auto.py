    ...
    def train(self, replay_buffer, batch_size=256, writer=None, writer_timestamp=None, invest=False):
        ...
        if self.total_it % self.policy_freq == 0:
            if len(self.his_policy_list) < self.reg_his_idx:
                ...
                reg_actions = self.actor(reg_data.observations)
                with torch.no_grad():
                    frozen_actions = reg_his_p(reg_data.observations)
                reg_loss = F.mse_loss(reg_actions, frozen_actions)

            # Compute actor loss
            q = self.qf1(data.observations, self.actor(data.observations))
            actor_loss = -q.mean()

            # NOTE - inspired by TD3+BC, but calculate adaptive coefficient while keep the scale of actor_loss
            total_policy_loss = actor_loss + self.reg_coef * reg_loss
            ...
            # NOTE - track the policy loss and reg loss
            self.p_loss_his_list.append(actor_loss.item())
            if len(self.his_policy_list) >= self.reg_his_idx:
                self.reg_loss_his_list.append(reg_loss.item())

            if self.total_it >= 10000:
                frac = 1.0 - (self.total_it - 1.0) / 1e6
                frac = max(frac, 0.1)
                running_p_loss = np.mean(np.abs(self.p_loss_his_list[-50000:]))
                running_reg_loss = np.mean(self.reg_loss_his_list[-50000:])
                self.reg_coef = self.target_rel_loss_scale * running_p_loss / (running_reg_loss + 1e-8) * frac
        ...
        if writer is not None:
            ...
            writer.add_scalar("charts/auto_reg_coef", self.reg_coef, writer_timestamp)
